defmodule HelloWeb.CommentControllerTest do
  ...
  @create_attrs %{comment: "some comment", likes: 42, reply_to: 42, topic_id: 42, user_id: 42}
  ...
  describe "index" do
    test "lists all topic_comments", %{conn: conn} do
      conn = get(conn, Routes.comment_path(conn, :index))
      assert html_response(conn, 200) =~ "Listing Topic comments"
    end
  end
